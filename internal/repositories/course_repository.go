...
func (r *CourseRepository) CreateNewCourse(course *model.Course) error {
	return r.db.Exec(`
		INSERT INTO courses 
...
func (r *CourseRepository) getModuleLessons(moduleID, userID string) ([]dtos.LessonResponse, error) {
	type LessonRaw struct {
		...
		Type      model.LessonType `json:"type"`
	}
...
// Lấy tổng thời gian học (dùng total_time trong courses)
func (cr *CourseRepository) GetCourseTotalTime(courseID uuid.UUID) (int, error) {
	var totalTime int
	err := cr.db.
		Table("courses").
		Select("COALESCE(total_time, 0)").
		Where("id = ?", courseID).
...
func (cr *CourseRepository) GetCourseModulesWithLessons(courseID uuid.UUID, userID uuid.UUID) ([]model.Module, map[uuid.UUID][]model.Lesson, map[uuid.UUID]string, error) {
	var modules []model.Module

	// Lấy tất cả các module của khóa học
	err := cr.db.Model(&model.Module{}).
		Preload("Lessons").
	...
	lessonsByModule := make(map[uuid.UUID][]model.Lesson)
	...
	for _, module := range modules {
		...
		for _, lesson := range module.Lessons {
			var lesson_attendance model.LessonAttendance
			err = cr.db.Model(&model.LessonAttendance{}).
...
func (cr *CourseRepository) CreateOrUpdateCourseRating(userID uuid.UUID, courseID uuid.UUID, star int, comment string) error {
	...
	result := cr.db.Where("user_id = ? AND courses_id = ?", userID, courseID).
		Assign(model.UserRate{Star: star, Comment: comment}).
	...
	var stars []int
	err := cr.db.Model(&model.UserRate{}).
		Where("courses_id = ?", courseID).
...
func (cr *CourseRepository) DeleteCourse(courseID uuid.UUID) error {
	...
	result := cr.db.Delete(&model.Course{}, courseID)
	if result.Error != nil {
		return result.Error
	}
...
func (cr *CourseRepository) RegisterUserToCourse(userIDs []uuid.UUID, courseID uuid.UUID, status model.UserCourseStatus) error {
	if len(userIDs) == 0 {
		return nil
	}
...
func (cr *CourseRepository) UpdateTotalTime(moduleID uuid.UUID, time int) error {
	var courseID string
	err := cr.db.Model(&model.Module{}).Where("id = ?", moduleID).Select("course_id").Scan(&courseID).Error
	if err != nil {
		return err
	}
	...
	return cr.db.Model(&model.Course{}).
		Where("id = ?", courseID).
		Update("total_time", gorm.Expr("GREATEST(total_time + ?, 0)", time)).Error
}

func (cr *CourseRepository) UpdateTotalLesson(moduleID uuid.UUID, lessonCount int) (*model.Course, error) {
...
func (cr *CourseRepository) GetModuleByID(moduleID uuid.UUID) (*model.Module, error) {
...
// 	var count int64
// 	err := cr.db.Model(&model.UserCourse{}).
...
func (cr *CourseRepository) IsCanLearn(userID, courseID uuid.UUID) bool {
	...
	return userCourse.Status == model.UserCourseStatusLearn
}

func (cr *CourseRepository) GetTotalCourseOfUser(userID uuid.UUID, status model.UserCourseStatus) (int64, error) {
	var total int64

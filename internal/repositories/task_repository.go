...
func (tr *TaskRepository) GetTasks(page, pageSize int, name, eventID string, isComplete *bool) ([]*model.Task, int64, error) {
...
func (tr *TaskRepository) CreateTask(createdBy, name, description, eventID string, startDate, deadline time.Time) (*model.Task, error) {
...
func (tr *TaskRepository) UpdateTask(id, name, description string, percent_complete float32, startTime, deadline time.Time) error {
	return tr.db.Model(&model.Task{}).Where("id = ?", id).Updates(model.Task{
		Name:            name,
		Description:     description,
...
func (tr *TaskRepository) DeleteTask(id string) error {
	return tr.db.Delete(&model.Task{}, "id = ?", id).Error
}

func (tr *TaskRepository) AddUserTask(taskID, userID uuid.UUID) (*model.TaskAssignments, error) {
	taskAssignment := &model.TaskAssignments{
		TaskID:      taskID,
		UserID:      userID,
		IsCompleted: false,
	}
...
func (tr *TaskRepository) UpdateTaskUserStatus(taskID, userID uuid.UUID, isCompleted bool) error {
	err := tr.db.Model(&model.TaskAssignments{}).
...
func (tr *TaskRepository) DeleteUserTask(taskID, userID uuid.UUID) error {
	taskAssign := &model.TaskAssignments{
		TaskID: taskID,
		UserID: userID,
	}

...
func (r *CourseRepository) GetCoursesV2(userID string, filter dtos.GetListCoursesForm) ([]dtos.GetListCoursesResponse, int64, error) {
	query := r.db.Model(&model.Course{}).Preload("Tags")
	if filter.Title != "" {
	...
	if filter.UserStatus != "" && userID != "" {
		...
		r.db.Model(&model.UserCourse{}).Select("course_id").Where("user_id = ? AND status = ?", userID, filter.UserStatus).Scan(&coursesIDs)
	...
	for _, course := range courses {
		...
		status := ""
		if userID != "" {
			err := r.db.Model(&model.LessonAttendance{}).
				Where("user_id = ? AND course_id = ? AND status = ?", userID, course.ID, string(model.Present)).
			...
			err = r.db.Model(&model.UserCourse{}).
...
func (r *CourseRepository) GetCourseDetailByIDV2(userID string, courseID string) (dtos.GetCourseDetailResponse, error) {
	...
	if userID != "" {
		var count int64
		r.db.Model(&model.FavoriteCourse{}).Where("user_id = ? AND course_id = ?", userID, courseID).Count(&count)
		if count > 0 {
			liked = true
		}
		var userCourse model.UserCourse
		err := r.db.Model(&model.UserCourse{}).Where("user_id = ? AND course_id = ?", userID, courseID).First(&userCourse).Error
		if err == nil {
			status = string(userCourse.Status)
		}
	}
	...
	err := r.db.Model(&model.UserRate{}).Where("courses_id = ?", courseID).Find(&rate).Error
	if err != nil {
	...
	for i, module := range course.Modules {
		for _, lesson := range module.Lessons {
			...
			var lessonAttendance *model.LessonAttendance
			err := r.db.Model(&model.LessonAttendance{}).

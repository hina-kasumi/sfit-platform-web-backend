...
func (ts *TaskService) GetTasks(page, pageSize int, name, eventID string, isCompleted *bool) ([]*model.Task, int64, error) {
	if page < 1 {
		page = 1
	}
...
func (ts *TaskService) CreateTask(createdBy, name, description, eventID string, startDate, deadline time.Time) (*model.Task, error) {
...
func (ts *TaskService) AddUserTask(userID, taskID string) (*model.TaskAssignments, error) {
	userIDParsed, err := uuid.Parse(userID)
	if err != nil {
		return nil, err
	}
